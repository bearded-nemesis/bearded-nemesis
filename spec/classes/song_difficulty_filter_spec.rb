...
describe SongDifficultyFilter do
  ...
  it "should match songs within a specified difficulty" do
    song = Struct::DummySong.new 3, 2
    filter = SongDifficultyFilter.new :bass, low: 2, high: 4
    filter.matches?(song).should be_true
  end

  it "should not match a song outside the specified difficulty" do
    song = Struct::DummySong.new 3, 2
    filter = SongDifficultyFilter.new :pro_drums, low: 3, high: 5
    filter.matches?(song).should be_false
  end
end