...
describe GenreFilter do
  ...
  it "should match songs with a specified genre" do
    song = Struct::DummyGenreSong.new "Rock"
    filter = GenreFilter.new "Rock"
    filter.matches?(song).should be_true
  end

  it "should not match a song with a nil genre" do
    song = Struct::DummyGenreSong.new nil
    filter = GenreFilter.new "Rock"
