...
(function() {
  ...
  $(function() {
    $('.add-ratings').click(function(evt) {
      var $modal, method, url;
      evt.preventDefault();
      url = $(this).attr("href");
    ...
    $(".star-rating a").click(function(evt) {
      var $wrapper;
      evt.preventDefault();
      $wrapper = $(this).closest(".star-rating");
      $wrapper.find("a").removeClass("current-rating");
      $wrapper.siblings("[type=hidden]").val($(this).text());
      return $(this).addClass("current-rating");
    });
    $('#ratings-modal form').ajaxForm(function() {
      return $("#ratings-modal").modal("hide");
    });
    $('#save-ratings').click(function() {
      return $('#ratings-modal form').submit();
    });
    $('.ownership').click(function(evt) {
      var does_own, link, url;
      evt.preventDefault();
      link = $(this);
      url = link.data("url");
      ...
      return $.post(url, {
        ...
        return link.text(does_own ? "No" : "Yes");
      });
    });
    return ko.applyBindings(rating);
  });

  ko.bindingHandlers.checkStarRating = {
    update: function(element, valueAccessor, allBindingsAccessor) {
      var allBindings, ratingIndex, value, valueUnwrapped;
      value = valueAccessor();
      allBindings = allBindingsAccessor();
      valueUnwrapped = ko.utils.unwrapObservable(value);
      ratingIndex = allBindings.ratingIndex;
      if (valueUnwrapped === ratingIndex) {
        return $(element).addClass("current-rating");
      } else {
        return $(element).removeClass("current-rating");
      }
    }
  };
  ...
  rating = {
    ...
    parse: function(data) {
      ...
      return this.overall(data.overall);
    }
  };
